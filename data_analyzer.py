...
class EnhancedRedFlagsAnalyzer:
    ...
    def __init__(self, input_parquet="data/all_billionaires.parquet", output_dir="data"):
    ...
    def _load_and_validate_data(self):
        ...
        if inflation_cols:
            for col in inflation_cols:
                ...
                self.logger.info(f"   {col.upper()}: {coverage:.1f}% coverage ({non_null_count:,} records)")
        else:
    ...
    def _calculate_enhanced_daily_aggregations(self, df):
        ...
        daily_data['total_wealth'] = daily_data['total_wealth'] / 1_000_000  # To trillions
        daily_data['average_wealth'] = daily_data['total_wealth'] / daily_data['billionaire_count'] * 1000  # To billions
        ...
        daily_data['wealth_inequality'] = daily_data['wealth_std'] / daily_data['average_wealth']
        
        self.logger.info(f"✅ Enhanced daily data: {len(daily_data)} points with {len(daily_data.columns)} metrics")
        return daily_data
    ...
    def _generate_enhanced_sparklines(self, daily_data):
        ...
        sparklines = {
            ...
            "inequality": [round(x, 3) for x in sampled_data['wealth_inequality'].tolist()],
            "bounds": {
                "wealth": {
    ...
    def _generate_enhanced_metadata(self, daily_data, enhanced_metrics):
        ...
        for col in ['cpi_u', 'pce']:
            if col in daily_data.columns:
                non_null_count = int(daily_data[col].notna().sum())  # Convert to native int
                inflation_status[col] = {
                    'available': bool(non_null_count > 0),  # Convert to native bool
                    'coverage_pct': float(round((non_null_count / len(daily_data)) * 100, 1)),  # Convert to native float
        ...
        metadata = {
            ...
            "trend_analysis": {
                "exponential_fit_r_squared": float(enhanced_metrics.get('exponential_fit', {}).get('r_squared', 0)),
                "annual_growth_rate": float(enhanced_metrics.get('exponential_fit', {}).get('annual_growth_rate', 0)),
                "fit_period_days": int(enhanced_metrics.get('exponential_fit', {}).get('fit_period_days', 0))
            }
        }
        ...
        chart_data = {
            "wealth_timeline": {
                ...
                "summary": {
                    ...
                    "exponential_growth_rate": enhanced_metrics.get('exponential_fit', {}).get('annual_growth_rate', 0)
                }
            },
            "count_timeline": {
                "data": [
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Enhanced Red Flags Profits Data Analysis")
    parser.add_argument(
        "--input",
